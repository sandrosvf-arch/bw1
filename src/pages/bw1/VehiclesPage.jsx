...
export default function VehiclesPage() {
  ...
  useEffect(() => {
    const fetchUpdatedListings = async () => {
      try {
        const response = await api.getListings({ category: 'vehicle', limit: SERVER_PAGE_SIZE });
  ...
  const handleClearSearch = async () => {
    ...
    setIsSearching(true);
    try {
      const response = await api.getListings({ category: 'vehicle', limit: SERVER_PAGE_SIZE }, { forceRefresh: true });
      const fetched = response.listings || [];
